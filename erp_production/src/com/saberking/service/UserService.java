...
//			Boolean b = true;
	...
	public String getNotices(String uid) {
		Singleton singleton = Singleton.getSingleton();
		List<String> messages = singleton.getMessages(uid);
		if (messages != null) {
	...
	public String receivedNotices(String uid) {
		Singleton singleton = Singleton.getSingleton();
		boolean res = singleton.receive(uid);
		if (res) {
			return "{\"msg\":\"success\"}";
		} else {
			return "{\"msg\":\"error\"}";
